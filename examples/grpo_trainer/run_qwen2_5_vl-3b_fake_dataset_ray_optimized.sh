#!/bin/bash
# Optimized training script for Qwen2.5-VL-3B-Instruct to fix Ray OOM issues
...
# ============================================================================
# RAY MEMORY OPTIMIZATION - CRITICAL FOR FIXING OOM
# ============================================================================

# Increase memory threshold before Ray kills workers (default is 0.95)
# This prevents Ray from being too aggressive in killing workers
export RAY_memory_usage_threshold=0.98

# /dev/shm is only 23GB in this container, so we limit object store to 20GB
# to stay within the shared memory limit
export RAY_object_store_memory=$((20 * 1024 * 1024 * 1024))

# Allow Ray to use slower storage if /dev/shm is exceeded
# This enables fallback to disk-based storage for object store
export RAY_OBJECT_STORE_ALLOW_SLOW_STORAGE=1

# Disable Ray memory monitor killing (optional, use with caution)
# export RAY_memory_monitor_refresh_ms=0

# Enable Ray verbose logging for debugging
...
echo "Ray object store memory limit: $((RAY_object_store_memory / 1024 / 1024 / 1024))GB"
echo "Ray memory usage threshold: $RAY_memory_usage_threshold"
...
# ============================================================================
# TRAINING CONFIGURATION WITH COMPREHENSIVE OOM FIXES
# ============================================================================
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen2_5_vl_3b_ray_optimized' \
    ...
    +ray_kwargs.ray_init._memory=$((100 * 1024 * 1024 * 1024))
